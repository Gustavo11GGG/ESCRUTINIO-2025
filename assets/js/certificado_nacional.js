...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // 💾 Guardar certificado nacional
    document.getElementById("guardarCertificado").addEventListener("click", async () => {
      ...
      await guardarResultado(resultadoMesa, "nacional");
    });
  } catch (err) {
    formularioContainer.innerHTML = `<p class="error">⚠ Error: ${err.message}</p>`;
    console.error("❌ Error al cargar fuerzas nacionales:", err);
  }
});
